...
function SearchResults(props) {
    ...
    const [products, setproducts] = useState(Object.values(props.data).filter((product) => product.name.toLowerCase().includes(params.query)))
    useEffect(() => {
        setproducts(Object.values(props.data).filter((product) => product.name.toLowerCase().includes(params.query)));
    }, [params])
    ...
    return (
        <Box sx={{ display: 'flex', mt: 10 }}>
            ...
            >
                <Box >
                    <Typography variant="h5" sx={{ display: "flex", alignItems: "center", justifyContent: "start", ml: 4, my: 4 }}>
                        Search Results
                    </Typography>
                </Box>
                <div style={{ display:"flex", justifyContent:"center", marginBottom:20}}>
                    <Pagination count={count} page={page} color="secondary" onChange={handleChange} />
                </div>
